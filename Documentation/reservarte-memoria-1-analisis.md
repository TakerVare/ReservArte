...
- **Ficha Customer (RA-869f1xc2n, PR #59, 2026-09-15):** `RegisterAsync` crea cuenta Identity y fila `Customer` (mismo Id, categoría `regular`) **dentro de `IUnitOfWork`**. Comprueba el `IdentityResult` de `CreateAsync`; si falla, no hay ficha ni cuenta. Contrato de `RegisterRequest` **sin cambio**. **Sin** consentimiento granular `data_processing` (el nivel a no lo implica; vol. 1 **§6.1.3**). Un email ya usado **en el tenant** (empleada u otra clienta) → 409 `GEN_CONFLICT`. El mismo email en **otra** org es válido (RA-869f1xc0u). No hay migración de backfill: el demo se rellenó en `DevSeeder` / `seed_demo`. Cuentas `Customer` antiguas sin ficha **no** se reparan solas.
...
2. Tras validar al sujeto en el IdP, el backend localiza o crea el usuario en Identity y registra el vínculo en **`AspNetUserLogins`** (PK `(OrganizationId, LoginProvider, ProviderKey)`; `OrganizationUserStore` sella el tenant de la cuenta). Tres caminos implementados (RA-869d7ez7e, 2026-07-18; alcance por org RA-869f1xc0u):
   - **Vínculo existente** en **esa organización** (`AspNetUserLogins` ya tiene el par proveedor/clave **en ese tenant**): se emiten tokens.
   ...
   - **Sin coincidencia**: alta **solo-social** con `PasswordHash` NULL, `EmailConfirmed = true` y `Rol = Roles.DefaultForPublicRegistration` (**`Customer`**, mismo que el registro local; RA-869f18116). **RA-869f1xc2n:** también crea la ficha `Customer` (mismo Id, `regular`) en la misma transacción `IUnitOfWork` que `CreateAsync` + `AddLoginAsync`. Comprueba cada `IdentityResult`. **Sin** `data_processing`. Los caminos de **vínculo existente** o **email coincidente** no tocan `Customers` (la cuenta ya existía; si era empleada, no gana ficha de clienta — eso es **RA-869d7f369**).
...
- **Decisión (RA-869f1xc2n, PR #59):** el alta de ficha **no** persiste `data_processing`. El nivel (a) no lo implica; inferirlo sería consentimiento tácito. `Required` del catálogo **no se relajó**. El registro **no** gana un checkbox extra (contrato intacto). El alta de ficha por el centro (RA-869d7f3bt / RA-869d7f3q4) deberá recabar `data_processing` de forma explícita.
