...
>
> **Hallazgo pre-arreglo (reproducido en `develop`):** `PUT` con el email de una cuenta **sin ficha** (admin) devolvía **200** y escribía el email crudo en ficha y `AspNetUsers`, dejando `NormalizedEmail` de la empleada. Causa: `EmailExistsAsync` solo mira `Employees`; los `IdentityResult` de `SetEmailAsync`/`SetUserNameAsync`/`UpdateAsync` se **ignoraban**; Identity muta el usuario en memoria **antes** de validar; el `SaveChanges` del repositorio persistía lo rechazado. Los índices únicos de normalizados no actuaron porque Identity rechaza **antes** de recalcularlos. No daba acceso a la cuenta del admin (el login usa el normalizado). Tras el arreglo: **409 `GEN_CONFLICT`**, datos intactos.
>
> **Alta sin contraseña (decisión de producto):** la cuenta se crea **sin credencial local**; quien da el alta nunca conoce la contraseña. **RA-869f17y68:** invitación (proveedor `Invitation`, 7 días). Canje: `POST /api/v1/auth/set-password`. Reenvío: `POST /api/v1/employees/{id}/invitation`. **La invitación se envía después del commit** (RA-869f1811u); un fallo de correo **no** revierte el alta. `LoginAsync` admite cuentas sin hash local. `forgot-password` queda para quien **ya** tiene contraseña.
>
...
- Sistema de categorización de clientes:
  - VIP (`vip`), Regular (`regular`), Nuevo (`new` — categoría con la que **nace toda ficha**; RA-869d7f369)
  - Promoción `new` → `regular` tras citas completadas: **RA-869f2g02q** (subtarea del bloque de Citas **RA-869d7edau**)
...
- CustomerNote: `EmployeeId` = ficha Employee activa de quien escribe (RA-869d7f3fw); un admin sin ficha no firma notas
...
- sin OrganizationId hoy (se añade al mapear: **RA-869f2gnbm**, Redsys; antes RA-869d7f3fw)
```
...
>
> **Esquema y repositorio (RA-869d7f32r, PR #58, 2026-09-15):** migración `20260915112149_AddCustomers` (solo crea tablas). Mapeadas `Customer`, `CustomerNote`, `CustomerAllergy`, `CustomerConsent` con query filter. **`CustomerPaymentMethod` sigue en `Ignore`** hasta RA-869d7f3fw. Índice único `IX_Customers_OrganizationId_Email`. CHECKs de catálogo generados desde constantes de dominio (`CatalogCheck`). `ICustomerRepository` en `ReservArte-Domain/Interfaces` (ClickUp pedía `Application/Interfaces`). **Sin `GetHistoryAsync`** (hace falta `Appointment`; `/history` es RA-869d7f3fw). Suite entonces **237/237**. **Trasladado el 2026-09-15:** `/history` → **RA-869f2gn91** (Citas); `CustomerPaymentMethod` y `/payment-methods` → **RA-869f2gnbm** (Redsys). Detalle: vol. 1 **§5.2**, vol. 2 **§9.7**.
>
> **Alta pública con ficha (RA-869f1xc2n, PR #59, 2026-09-15):** `POST /api/v1/auth/register` crea cuenta + ficha `Customer` (mismo Id, `PreferredContactMethod = email`) + consentimiento `data_processing` (`IsGranted = 1`, `GrantedAt`) en `IUnitOfWork`. En **ese** PR la categoría era `regular`; **desde RA-869d7f369** toda ficha nueva (registro, alta social y alta desde el centro) nace **`new`**. Campo nuevo **`acceptedDataProcessing`** (bool obligatorio). Ausente o `false` → 400 `GEN_VALIDATION_FAILED` (`details[].field = "acceptedDataProcessing"`). Email repetido en el centro → 409 `GEN_CONFLICT`, sin ficha duplicada. Tokens **tras el commit**. Alta social de cuenta **nueva**: cuenta + vínculo (`AddLoginAsync` comprobado) + ficha, **sin** consentimientos granulares. Vincular a una cuenta existente **no** toca fichas. Migración `20260915151444_BackfillCustomerProfiles` (solo SQL; `Down()` vacío; esas fichas rellenadas siguen siendo `regular`). Suite entonces **246/246**, E2E **57/57**. Recuento entonces: **3/8**. Siguiente entonces: **RA-869d7f369**. Detalle: vol. 1 **§4.4.1**, **§6.1.3**, vol. 2 **§9.7**.
>
...
- Transiciones de estado (**RA-869d7f4xf**, servicio; rutas HTTP: **RA-869d7f519**):
  ...
  - cancelación desde cualquier estado **vivo** (`CancelAsync`) — personal o clienta **dueña**; una clienta sobre cita ajena → **404** (no 403)
  - no-show desde cualquier estado vivo (`MarkNoShowAsync`) — solo Admin o Manager
...
2. **Identificación de tenant:**
   ...
   - El claim JWT `organization_id` **no** sustituye la resolución: con autenticación activa, `TenantMiddleware` **verifica** que coincida con el tenant resuelto por cabecera/subdominio. **No resuelta / desconocida → 400** `ORG_TENANT_NOT_RESOLVED` (corregir contexto). **Resuelta pero distinta del claim → 403** `ORG_TENANT_MISMATCH` (cerrar sesión o reautenticar). Verificado en runtime 2026-09-13 (PR #42; org temporal luego eliminada). Requiere `UseAuthentication()` **antes** del middleware (`Program.cs`).
...
**Modelo de usuario (ASP.NET Core Identity, decisión RA-869d7eyvf, 2026-07-06; logins por org RA-869f1xc0u, 2026-09-15):**
- La entidad `User` extiende `IdentityUser<int>`; `AppDbContext` hereda de `IdentityUserContext<User, int, IdentityUserClaim<int>, UserLogin, IdentityUserToken<int>>` (variante **sin roles** de Identity: el proyecto usa el campo de negocio `Rol` string, no `AspNetRoles`). `UserLogin : IdentityUserLogin<int>` añade `OrganizationId`.
...
- Email: columnas `Email` y `NormalizedEmail`; índice único `EmailIndex` sobre **`(OrganizationId, NormalizedEmail)`** (`RequireUniqueEmail = true`, validación por tenant). `UserNameIndex` = `(OrganizationId, NormalizedUserName)`. Ambos filtrados `IS NOT NULL`. El store de Identity es **`OrganizationUserStore`**: al vincular un proveedor, el vínculo hereda la organización de la cuenta.
...
3. Si el usuario **no** tiene 2FA: emisión directa de **access JWT** + **refresh token**
4. Si `TwoFactorEnabled`: **no** se emiten tokens. La respuesta lleva `MfaRequired = true` y un **ticket intermedio** (`MfaTicket`): JWT de **5 min**, claim `mfa_pending`, **sin** claim `role`. Ese ticket **no autoriza** operaciones (`OnTokenValidated` lo rechaza en cualquier `[Authorize]`)
5. `POST /api/v1/auth/mfa/verify` con el ticket + código (TOTP o de recuperación de un solo uso) → emisión del par definitivo access + refresh
...
**Registro local (`POST /api/v1/auth/register`) — decisión RA-869d7ez3e (2026-07-17); consentimiento RGPD RA-869epf0rt (2026-08-25); rol RA-869f18116 (2026-09-13); ficha Customer RA-869f1xc2n (2026-09-15):**
...
- **Ficha Customer (RA-869f1xc2n, PR #59; categoría RA-869d7f369):** `RegisterAsync` crea cuenta Identity + ficha `Customer` (mismo Id; nombre, apellidos, email y teléfono de la cuenta; `Category = new`; `PreferredContactMethod = email`) + consentimiento `data_processing` (`IsGranted = 1`, `GrantedAt`) en **una transacción** `IUnitOfWork.ExecuteInTransactionAsync` (patrón RA-869f1811u; `AuthService` recibe `IUnitOfWork`). En PR #59 la categoría era `regular`. Se comprueba cada `IdentityResult`. Email repetido en el centro → **409 `GEN_CONFLICT`**, sin ficha duplicada. **Los tokens se emiten tras el commit**: si fallara la emisión, la cuenta ya existe y la persona puede iniciar sesión. El mismo email **sí** puede registrarse en dos organizaciones (RA-869f1xc0u).
- **Contrato — `acceptedDataProcessing`:** el `RegisterRequest` exige este bool **además** de `acceptedTerms` y `acceptedPrivacy`. Si falta o llega a `false`: **400 `GEN_VALIDATION_FAILED`** con `details[].field = "acceptedDataProcessing"` y mensaje «Debes aceptar el tratamiento de tus datos para gestionar tus citas.». Validación en `RegisterRequestValidator` y guarda en `AuthService` (nunca persiste un consentimiento no marcado). **Cambio de contrato:** cualquier cliente del endpoint debe enviarlo. La SPA ya lo hace (`RegisterForm`, `register.schema.ts`, `RegisterCredentials`, `RegisterPage`).
...
- **Consentimiento RGPD (obligatorio en el alta local):** el `RegisterRequest` incluye `AcceptedTerms`, `AcceptedPrivacy`, **`AcceptedDataProcessing`**, `AcceptedTermsVersion` y `AcceptedPrivacyVersion`. FluentValidation exige los tres flags a `true` y versiones no vacías. El cliente obtiene las vigentes con **`GET /api/v1/legal/versions`** (público, envelope `{ termsVersion, privacyVersion }`) y las envía en el registro; el backend compara con `LegalDocuments:TermsVersion` / `PrivacyVersion` y rechaza con `GEN_VALIDATION_FAILED` si no coinciden (p. ej. documentos actualizados o cliente con versión cacheada). Si coinciden, persiste en el usuario las versiones aceptadas y `ConsentAcceptedAt` (UTC), y en `CustomerConsents` el `data_processing` marcado. **Fail-fast al arranque:** `ValidateOnStart` exige que ambas versiones no estén vacías; si faltan, la API **no arranca** (mensaje claro). Evita un fallo silencioso del registro por configuración olvidada (vol. 1 **§5.1.3**). **SPA (`RegisterPage`, RA-869d7fbhg + RA-869f1xc2n):** carga las versiones al montar, **tres** checkboxes (términos + privacidad + tratamiento de datos; enlaces a `/legal/terminos` y `/legal/privacidad`, **públicas** y stub; el de `data_processing` **sin enlace**) y login automático tras el alta; ver vol. 2 **§9.2.3**.
...
1. **`POST /api/v1/auth/forgot-password`** `{ email }`. Identity busca el usuario **del tenant de la petición**. Si el email pertenece a esa organización, genera el token (`GeneratePasswordResetTokenAsync`). El token se **codifica para URL** (`Uri.EscapeDataString`) y se envía en el **enlace del email** como segmento `{App:FrontendBaseUrl}/reset-password/{token}`. El token de un centro **no vale** en otro. SPA: `ForgotPasswordPage` / `ResetPasswordPage` (**RA-869d7fbmy**); ruta `/reset-password/:token?`. Vol. 2 **§9.2.3**.
2. **`POST /api/v1/auth/reset-password`** `{ email, token, newPassword }`. El email lo introduce el usuario en el formulario (no viaja en la URL).
3. **Contrato del token (RA-869f18rp7, 2026-09-13):** en el enlace va **URL-encoded**; Vue Router **decodifica** `route.params.token`; el **body del POST lleva el token en claro, decodificado exactamente una vez**. `Uri.UnescapeDataString` en el backend se **conserva como tolerancia** (inocuo sobre base64: no transforma `+`, a diferencia de `UrlDecode`; base64 no contiene `%`). Fijado por `reservarte-web/e2e/reset-password.spec.ts` (token sintético con `+`, `/`, `=`; 3 navegadores) más el caso «sin token → no formulario».
4. **Anti-enumeración en ambos extremos:** forgot responde **siempre 200** con mensaje genérico. Reset: usuario inexistente, otra organización o token inválido/caducado → mismo error opaco (`AUTH_INVALID_CREDENTIALS`). El token emitido para la cuenta de un centro **no** canjea en otro. Si la nueva contraseña no cumple la política de Identity, **`GEN_VALIDATION_FAILED`** con detalle en `newPassword`.
...
> **Verificado:** (a) runtime 2026-09-13, primera vez forgot → fichero local → reset; (b) contrato de decodificación **automatizado** (PR #42). El flujo completo contra backend real **no** está en Playwright (el spec intercepta el POST). Ampliación: **RA-869f18uta** (se cruza con E2E en CI, **RA-869eqxm7w**).

**Invitación al alta de empleado (RA-869f17y68, PR #51, 2026-09-14):** no reutiliza el token de recuperación (ese proveedor rechazaría el de invitación).

1. **Proveedor Identity `Invitation`**, propósito **`SetPassword`**, caducidad **7 días**. `DataProtectionTokenProviderOptions` es **compartido**; no se toca el de reset (**1 día**). Constantes en `InvitationTokenDefaults` (`ReservArte-Application/Common`); `InvitationTokenProvider` + options en **`ReservArte-API/Identity`** (Infrastructure no referencia `Microsoft.AspNetCore.App`). Token **no persistido**: Data Protection + security stamp.
2. **Alta:** `CreateAsync` envía la invitación **después del commit** de la transacción ficha+cuenta (RA-869f1811u). Enlace `{App:FrontendBaseUrl}/set-password/{token URL-encoded}`. Texto plano (plantilla HTML común: **RA-869d7f65a**, SES). Asunto de bienvenida; el cuerpo avisa de 7 días y del reenvío. **Fallo de correo ≠ rollback** del alta (queda reenviable). El token **nunca** va a logs.
3. **`POST /api/v1/employees/{id}/invitation`:** mismas reglas de rol que el resto de escrituras (Manager no toca Admin → 403; 404 de tenant antes). **409 `GEN_CONFLICT`** si está de baja o **ya tiene contraseña** (eso es `forgot-password`). Fallo de envío → **500 `GEN_INTERNAL_ERROR`**. Éxito → 200 `EmployeeDto`.
4. **`POST /api/v1/auth/set-password`** `{ email, token, newPassword }` (anónimo): usuario de la org, **sin** contraseña, token `Invitation`/`SetPassword`, `AddPasswordAsync`, **renueva security stamp** (un solo uso). Fallos de negocio → 401 opaco `AUTH_INVALID_CREDENTIALS`. Política inválida → 400 `field = newPassword`. Mismo contrato de token que el reset (SPA una decodificación; backend `Uri.UnescapeDataString`). El token de un centro **no vale** en otro. SPA: `/set-password/:token?` (`SetPasswordPage`). **`EmailConfirmed` no se toca** (**RA-869f1812p**).

**Email — `IEmailService` (contrato activo, Application):** `Task SendAsync(EmailMessage message, CancellationToken)` con `To`, `Subject`, `Body`, `IsHtml`. Independiente del proveedor. El cuerpo se **construye en código** (reset e invitación en texto plano; **no** plantillas nativas de SES/SMTP). Personalización por tenant (nombre/marca): **RA-869d7f65a**. **DI por entorno (arranque seguro):** Development → `DevFileEmailService` (`./sent-emails/`); **fuera de Development**, si no hay implementación real, la API **no arranca**. Producción: registrar SES en esa rama DI. Vol. 2 **§8.1.1**.
...
2. Tras validar al sujeto en el IdP, el backend localiza o crea el usuario en Identity y registra el vínculo en **`AspNetUserLogins`** (PK `(OrganizationId, LoginProvider, ProviderKey)`; `OrganizationUserStore` sella el tenant de la cuenta). Tres caminos implementados (RA-869d7ez7e, 2026-07-18; alcance por org RA-869f1xc0u):
   ...
   - **Sin coincidencia**: alta **solo-social** con `PasswordHash` NULL, `EmailConfirmed = true` y `Rol = Roles.DefaultForPublicRegistration` (**`Customer`**, mismo que el registro local; RA-869f18116). **RA-869f1xc2n:** cuenta + vínculo (`AddLoginAsync` **comprobado**; antes se ignoraba) + ficha `Customer` en una transacción (`Category = new` desde RA-869d7f369; en PR #59 era `regular`). **Sin consentimientos granulares**: el flujo OAuth no pasa por el formulario. `data_processing` queda pendiente de recabar en otro punto (p. ej. primera reserva; no hay tarea ClickUp).
   La **misma** cuenta de Google/Apple/Meta entra en **dos centros**: en cada uno vincula a la cuenta local con ese email o crea una solo-social. Ante fallos de vinculación, las respuestas son **opacas** (no revelan detalle interno).
   **Limitación conocida (consentimiento RGPD, RA-869epf0rt + RA-869f1xc2n):** el alta por login social **no** recaba hoy el consentimiento base de alta ni `data_processing`. `AcceptedTermsVersion`, `AcceptedPrivacyVersion` y `ConsentAcceptedAt` quedan **NULL**. No es el estado deseado; la recogida de consentimiento en el flujo OAuth queda como **tarea de backlog**.
...
- Alta en dos pasos (usuario autenticado, `[Authorize]`):
  - `POST /api/v1/account/mfa/enable` — genera (o regenera) el secreto; responde `otpauthUri` + `manualEntryKey`; **no** activa `TwoFactorEnabled`. Cada llamada regenera el secreto: no rellamar tras el último enable o la app autenticadora queda desincronizada.
...
**Endpoints REST adicionales (MFA / cuenta — RA-869d7eze3 + RA-869d7ezgy):**
```
GET    /api/v1/account/me                             # claims del JWT; primer [Authorize]
POST   /api/v1/auth/mfa/verify                        # ticket + TOTP o código de recuperación → tokens
...
POST   /api/v1/account/mfa/confirm                    # activa 2FA + recoveryCodes (una sola vez)
...
**Datos sensibles:**
- Contraseñas locales: hash seguro con el **hasher de ASP.NET Core Identity (PBKDF2)** en `PasswordHash` (`AspNetUsers`); usuarios solo sociales pueden tener `PasswordHash` NULL. Complejidad del alta local: **dos capas coincidentes** (FluentValidation + defaults de Identity) y réplica Zod en RegisterPage; ver vol. 1 **§4.4.1** y vol. 2 **§9.2.3**.
...
```
# Autenticación
POST   /api/v1/auth/register   # acceptedTerms + acceptedPrivacy + acceptedDataProcessing (bool obligatorio, RA-869f1xc2n); 400 GEN_VALIDATION_FAILED field=acceptedDataProcessing
...
POST   /api/v1/employees/{id}/invitation      # reenvía invitación; 409 si baja o ya tiene contraseña; fallo de envío → 500; 403 si Manager toca Admin
...
GET    /api/v1/customers/{id}              # Admin|Manager|Employee; 200 CustomerDetailDto (consents, allergies, notes vigentes) | 404 GEN_NOT_FOUND
POST   /api/v1/customers                   # Admin|Manager; 201 + Location | 400 (grantedConsents sin data_processing → field=grantedConsents; catálogos) | 409 (email con ficha)
...
POST   /api/v1/services/categories                 # Admin|Manager; 201 + Location a GET /categories (no hay GET por id) | 400 field=name | field=displayOrder
PUT    /api/v1/services/categories/{categoryId}    # Admin|Manager; 200 | 400 | 404; no toca la baja
DELETE /api/v1/services/categories/{categoryId}    # Admin|Manager; 200 baja lógica idempotente; se permite aunque tenga servicios (conservan categoryId)
POST   /api/v1/services/categories/{categoryId}/reactivate  # Admin|Manager; 200 idempotente
POST   /api/v1/services/{id}/variations            # Admin|Manager; 201 + Location al detalle del servicio | 400 field=name | field=durationModifier (duración resultante ≤ 0) | 404 servicio
PUT    /api/v1/services/{id}/variations/{variationId}  # Admin|Manager; 200 | 400 | 404 (otro servicio o inexistente)
DELETE /api/v1/services/{id}/variations/{variationId}  # Admin|Manager; 200 baja lógica idempotente
...
> **Contrato `POST /api/v1/auth/register` (RA-869f1xc2n, PR #59):** además de `acceptedTerms` / `acceptedPrivacy` (y versiones), el body exige **`acceptedDataProcessing` (bool)**. Ausente o `false` → 400 `GEN_VALIDATION_FAILED`, `details[].field = "acceptedDataProcessing"`, mensaje «Debes aceptar el tratamiento de tus datos para gestionar tus citas.». **Cambio de contrato:** cualquier cliente del endpoint debe enviarlo. La SPA ya lo hace. Detalle: vol. 1 **§4.4.1**.
...
| **App** | `FrontendBaseUrl` | URL base del SPA para enlaces emitidos por la API (reset: `{FrontendBaseUrl}/reset-password/{token}`; invitación: `{FrontendBaseUrl}/set-password/{token}`). **Vacío** en `appsettings.json`. **Fail-fast:** `ValidateOnStart` (`AppOptions`) — si falta, la API **no arranca**. Development: origen del frontend en `appsettings.Development.json` (no documentar un puerto de máquina concreto). Producción: variable de entorno `App__FrontendBaseUrl` (u almacén de configuración). **v1:** URL única. **Fase 3 (multi-tenant):** el enlace deberá usar el subdominio de la organización, no esta URL global. RA-869eq5tg3 + RA-869f17y68. | No secreto. |
...
**`CancelledByType`** (`AppointmentCancelledByTypes`: `customer`, `business`) **se mantiene**. El mismo dato vive en dos columnas. **`Status` es la fuente de verdad.**

**Dos capas (RA-869d7f4xf, PR #76):**

- **Servicio.** Ningún camino de código puede escribir un par contradictorio. Quién cancela decide el estado y el tipo se rellena a juego, en la misma operación: personal → `cancelled_by_business` / `business`; clienta dueña → `cancelled_by_customer` / `customer`. El genérico `cancelled` **no lo escribe nadie** (decisión del usuario): se sigue **aceptando al leer** (el CHECK lo admite y puede venir de una importación) y sigue liberando hueco. Si un proceso automático (p. ej. expiración de pago) cancela algún día, habrá que decidir si usa el genérico con tipo nulo.
- **Base.** El CHECK valida cada columna por separado. Un `UPDATE` a mano **sí** puede dejar `cancelled_by_customer` con `CancelledByType = business` (verificado en runtime). No es un fallo del servicio.
