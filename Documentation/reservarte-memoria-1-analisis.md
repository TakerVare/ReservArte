...
>
> **Alta pública con ficha (RA-869f1xc2n, PR #59, 2026-09-15):** `POST /api/v1/auth/register` crea cuenta + ficha `Customer` (mismo Id, `Category = regular`, `PreferredContactMethod = email`) + consentimiento `data_processing` (`IsGranted = 1`, `GrantedAt`) en `IUnitOfWork`. Campo nuevo **`acceptedDataProcessing`** (bool obligatorio). Ausente o `false` → 400 `GEN_VALIDATION_FAILED` (`details[].field = "acceptedDataProcessing"`). Email repetido en el centro → 409 `GEN_CONFLICT`, sin ficha duplicada. Tokens **tras el commit**. Alta social de cuenta **nueva**: cuenta + vínculo (`AddLoginAsync` comprobado) + ficha, **sin** consentimientos granulares. Vincular a una cuenta existente **no** toca fichas. Migración `20260915151444_BackfillCustomerProfiles` (solo SQL; `Down()` vacío). Suite **246/246**, E2E **57/57**. Recuento del padre: **3/8**. Siguiente: **RA-869d7f369**. Detalle: vol. 1 **§4.4.1**, **§6.1.3**, vol. 2 **§9.7**.
>
> **Empleada y clienta, misma cuenta (decisión de producto 2026-09-14):** una empleada puede ser clienta de **su propio centro** con **la misma** cuenta. Un `User` puede tener ficha `Employee` y ficha `Customer` con el **mismo Id** (PK compartida). `User.Rol` sigue siendo el rol de **personal** (`Admin` / `Manager` / `Employee`). Consecuencias a implementar en **RA-869d7f369** (además de la categoría `new`): (1) la ficha Customer **no** implica `Rol = Customer`; (2) dar de alta como cliente a alguien que ya tiene cuenta en el tenant **añade la ficha**, no crea otra cuenta; (3) la baja de la ficha de cliente **no** bloquea la cuenta; el lockout solo lo hace la baja de empleado (RA-869f180e5). **Hoy** el registro público con un email ya usado en el centro responde **409** y no añade ficha; dar ficha de clienta a una cuenta existente es de **RA-869d7f369**.
>
> **Pendientes:** contador de no-shows **no** está en `Customer` — **RA-869d7f3ka**. Categoría `new` + dual ficha — **RA-869d7f369** (**siguiente** del bloque). `CustomerPaymentMethod.OrganizationId` — **RA-869d7f3fw**. Alta pública con ficha — **RA-869f1xc2n** (**shipped**, PR #59). Unicidad de email por org — **RA-869f1xc0u** (**shipped**, PR #57; independiente, no suma al 8). Cadena: **RA-869f1xc0u** (hecha) → **RA-869d7f32r** (hecha) → **RA-869f1xc2n** (hecha).
>
...
- **Ficha Customer (RA-869f1xc2n, PR #59):** `RegisterAsync` crea cuenta Identity + ficha `Customer` (mismo Id; nombre, apellidos, email y teléfono de la cuenta; `Category = regular`; `PreferredContactMethod = email`) + consentimiento `data_processing` (`IsGranted = 1`, `GrantedAt`) en **una transacción** `IUnitOfWork.ExecuteInTransactionAsync` (patrón RA-869f1811u; `AuthService` recibe `IUnitOfWork`). Se comprueba cada `IdentityResult`. Email repetido en el centro → **409 `GEN_CONFLICT`**, sin ficha duplicada. **Los tokens se emiten tras el commit**: si fallara la emisión, la cuenta ya existe y la persona puede iniciar sesión. El mismo email **sí** puede registrarse en dos organizaciones (RA-869f1xc0u).
...
2. Tras validar al sujeto en el IdP, el backend localiza o crea el usuario en Identity y registra el vínculo en **`AspNetUserLogins`** (PK `(OrganizationId, LoginProvider, ProviderKey)`; `OrganizationUserStore` sella el tenant de la cuenta). Tres caminos implementados (RA-869d7ez7e, 2026-07-18; alcance por org RA-869f1xc0u):
   ...
   - **Email coincidente** con un usuario de la **misma organización**: vinculación automática del proveedor al usuario existente y emisión de tokens; **no toca fichas**. Dar ficha de clienta a una cuenta ya existente es **RA-869d7f369**.
   - **Sin coincidencia**: alta **solo-social** con `PasswordHash` NULL, `EmailConfirmed = true` y `Rol = Roles.DefaultForPublicRegistration` (**`Customer`**, mismo que el registro local; RA-869f18116). **RA-869f1xc2n:** cuenta + vínculo (`AddLoginAsync` **comprobado**; antes se ignoraba) + ficha `Customer` en una transacción. **Sin consentimientos granulares**: el flujo OAuth no pasa por el formulario. `data_processing` queda pendiente de recabar en otro punto (p. ej. primera reserva; no hay tarea ClickUp).
