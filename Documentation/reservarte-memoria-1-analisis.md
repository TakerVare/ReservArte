...
2. Tras validar al sujeto en el IdP, el backend localiza o crea el usuario en Identity y registra el vínculo en **`AspNetUserLogins`** (PK `(OrganizationId, LoginProvider, ProviderKey)`; `OrganizationUserStore` sella el tenant de la cuenta). Tres caminos implementados (RA-869d7ez7e, 2026-07-18; alcance por org RA-869f1xc0u):
   ...
   - **Email coincidente** con un usuario de la **misma organización**: vinculación automática del proveedor al usuario existente y emisión de tokens; **no toca fichas**. Dar ficha de clienta a una cuenta ya existente está **implementado en el servicio** (RA-869d7f369, PR #60); **expuesto por API en RA-869d7f3bt**.
...
# Clientes — contrato de producto; API aún no (RA-869d7f3bt). Persistencia sí (RA-869d7f32r). Servicio sí (RA-869d7f369).
...
> **Capa de servicio (RA-869d7f369, PR #60) — endpoint pendiente (RA-869d7f3bt).** Las rutas de arriba **no** están implementadas. Contrato previsto en el servicio (no HTTP aún): alta con `grantedConsents` (valores de `CustomerConsentTypes`); sin `data_processing` → 400 `GEN_VALIDATION_FAILED` (`field = grantedConsents`). Email de otra ficha del centro → 409 `GEN_CONFLICT`. Cuenta de personal (`User.Rol != Customer`): cambiar el email → 403 `GEN_FORBIDDEN`. Perfil inexistente u otro centro → 404 `GEN_NOT_FOUND`. Sin tenant → 400 `ORG_TENANT_NOT_RESOLVED`. Lista: `GetPagedAsync` (mismo nombre que Empleados; ClickUp decía `SearchAsync`).
