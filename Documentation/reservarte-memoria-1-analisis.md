...
>
> **Alta sin contraseña (decisión de producto):** la cuenta se crea **sin credencial local**; quien da el alta nunca conoce la contraseña. **RA-869f17y68 (PR #51, 2026-09-14):** el alta envía un email de invitación (proveedor Identity `Invitation`, propósito `SetPassword`, **7 días**; el de recuperación sigue en **1 día**). Canje: `POST /api/v1/auth/set-password`. Reenvío: `POST /api/v1/employees/{id}/invitation`. Un fallo de correo **no** revierte el alta. Encaja en auth: `LoginAsync` ya admite cuentas sin hash local (mismo estado que el alta social) y no exige consentimiento RGPD en el login. `forgot-password` queda para quien **ya** tiene contraseña.
>
...
2. **Alta:** `CreateAsync` envía **después** de crear la ficha. Enlace `{App:FrontendBaseUrl}/set-password/{token URL-encoded}`. Texto plano (plantilla HTML común: **RA-869d7f65a**, SES). Asunto de bienvenida; el cuerpo avisa de 7 días y del reenvío. **Fallo de correo ≠ rollback** del alta (queda reenviable). El token **nunca** va a logs.
