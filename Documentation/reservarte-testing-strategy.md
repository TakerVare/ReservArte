...
>
> **Fin de sesión (2026-09-13, RA-869f18urw):** spec `reservarte-web/e2e/session-ending.spec.ts` — 403 + `ORG_TENANT_MISMATCH` cierra sesión; 403 con otro código (`GEN_FORBIDDEN`) no. Las llamadas se provocan **desde la app** (un `fetch` suelto no atraviesa el interceptor). Suite Playwright **36/36**.
>
...
| E2E | **Playwright** (TypeScript) + **`@axe-core/playwright`** | `reservarte-web/playwright.config.ts` y `reservarte-web/e2e/` (tres navegadores) — **RA-869eqxdk3**. Specs actuales: a11y LoginPage, OAuth callback, reset-password, session-ending. Suite **36/36** (2026-09-13). `tests/ReservArte.E2ETests` **abandonado**. Escenarios de producto **pendientes**. Forgot→reset con API real: **RA-869f18uta**. |
