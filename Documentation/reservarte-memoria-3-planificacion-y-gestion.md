...
- ✅ Autenticación básica
  ...
  - **Test a11y `LoginPage` (RA-869d7fbpp, 2026-09-11)** — **shipped.** Spec `reservarte-web/e2e/login.a11y.spec.ts`: WCAG 2.1 AA con `@axe-core/playwright` en tres estados (inicial, error, CAPTCHA). **No es conformidad plena:** el test **excluye** `color-contrast` (marca rosa `#FFB6C1`, ~1.62 vs 4.5:1); deuda **RA-869f0v6vm**. Bloque padre **RA-869d7edpt:** **6/7** (layouts + LoginPage + MfaVerifyPage + RegisterPage + Forgot/Reset + test a11y LoginPage). **Pendiente del bloque:** solo **OAuthCallback (RA-869d7f7r1)**, bloqueada por credenciales OAuth reales.
...
- ✅ Panel de administración — **navegación de diseño: solo BottomNav** (RA-869ep9b52); ver vol. 2 §9.2.4
  ...
  - **Páginas de auth implementadas (`LoginPage`, `MfaVerifyPage`, `RegisterPage`, `ForgotPasswordPage`, `ResetPasswordPage`):** patrón **Banner + contenido centrado**, **no** `AuthLayout`. `BottomNav` global en `App.vue` (3 destinos).
...
- ✅ Login **frontend** local (`LoginPage`, RA-869d7f7kn) + verificación 2FA (`MfaVerifyPage`, RA-869d7f7vw) + registro (`RegisterPage`, RA-869d7fbhg) + recuperación (`ForgotPasswordPage` / `ResetPasswordPage`, RA-869d7fbmy) + test a11y LoginPage (RA-869d7fbpp): shipped. Bloque RA-869d7edpt **6/7**. **Pendiente del bloque:** OAuthCallback (**RA-869d7f7r1**, bloqueada por credenciales OAuth reales). Turnstile real sigue pendiente (no es el 7.º ítem del bloque). El test a11y **no** certifica contraste AA (deuda RA-869f0v6vm).
...
- [ ] Vista de retorno OAuth (`OAuthCallback`, **RA-869d7f7r1**) — cableado correcto; **bloqueada por credenciales OAuth reales** (no shipped). Único pendiente del bloque Auth UI **RA-869d7edpt** (6/7).
...
>
> **Superado el 2026-09-11 — test a11y LoginPage (RA-869d7fbpp):** spec shipped (tres estados; `color-contrast` excluido; deuda RA-869f0v6vm). Bloque RA-869d7edpt: **6/7**. **Pendiente del bloque:** OAuthCallback (RA-869d7f7r1), bloqueada por credenciales OAuth reales. Widget Turnstile real pendiente (hueco y umbral 3 hechos). Backend reset: vol. 1 **§4.4.1** (RA-869eq5tg3). Patrón SPA + Zod: vol. 2 **§9.2.3**.
...
- [x] **E2E frontend:** **Playwright** + **`@axe-core/playwright`** en `reservarte-web` (`playwright.config.ts`, tests en `reservarte-web/e2e/`, Chromium / Firefox / WebKit). Scripts `test:e2e`, `test:e2e:ui`, `test:e2e:report`. Humo E2E y **test a11y `LoginPage` (RA-869d7fbpp)** verificados. Plan previo `tests/ReservArte.E2ETests` **abandonado**. Escenarios de producto E2E **siguen pendientes**. El test a11y **excluye** `color-contrast` (deuda RA-869f0v6vm).
