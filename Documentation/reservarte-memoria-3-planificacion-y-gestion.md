...
>
> **RA-869f1xc2n → shipped (2026-09-15), PR #59.** Alta pública con ficha `Customer`. `RegisterAsync` / `ExternalLoginAsync` en `IUnitOfWork`; categoría `regular`; sin `data_processing`; contrato de registro intacto. Sin migración de backfill (`DevSeeder` / `seed_demo`). Unit **243/243**. E2E **51/51**. Runtime: registro 201 + misma Id, segundo 409. Cadena: **RA-869f1xc0u** (hecha) → **RA-869d7f32r** (hecha) → **RA-869f1xc2n** (hecha) → **RA-869d7f369**.
>
...
- ⏳ CRUD de clientes — bloque **RA-869d7ed68: 3/8** (dominio + esquema/repositorio + alta pública; 2026-09-15)
  - **Entidades Domain (RA-869d7f2z5, 2026-09-14)** — **shipped** (PR #56). PK compartida, `OrganizationId` Guid, catálogos snake_case, sin `Rol`/`MarketingConsent` en ficha. Dual ficha empleada+clienta: **RA-869d7f369**. Detalle: vol. 1 **§3.1.3**, vol. 2 **§9.7**.
  ...
  - **Alta pública con ficha (RA-869f1xc2n, 2026-09-15)** — **shipped** (PR #59). Registro y OAuth (alta nueva) crean cuenta + `Customer` en transacción.
...
- Gestión de maestros: **empleados backend 10/10**; **clientes 3/8 (dominio + esquema/repositorio + alta pública)**; **servicios no empezado**. UI de empleados/clientes/servicios **no**.
...
- [x] **Backend unitario:** proyecto `tests/ReservArte.UnitTests` con xUnit + Moq + FluentAssertions; suite **243/243** (2026-09-15). Repositorios: SQLite en memoria. `[reservarte-testing-strategy.md](reservarte-testing-strategy.md)` §3.1
