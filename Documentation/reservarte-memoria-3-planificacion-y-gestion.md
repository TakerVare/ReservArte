...
>
> **Advertencia — `EnsureSlotAvailableAsync` no mira el reloj.** Asimetría deliberada con la consulta de huecos. **RA-869d7f4xf** no tocó el alta. Si el pasado se admite al registrar, lo decide **RA-869d7f519**.
>
> **Advertencia — mutaciones como práctica:** los 36 casos de disponibilidad y los 38 de la máquina de estados se contrastaron cada uno con cinco mutaciones deliberadas y las cinco cayeron. Es la forma de no dar por bueno un verde de adorno.
>
> **Advertencia — ocho valores en `Status` Y se mantiene `CancelledByType` (decisión del usuario):** el mismo dato vive en dos columnas. **`Status` es la fuente de verdad.** El servicio (**RA-869d7f4xf**) impone la coherencia al cancelar: ningún camino de código escribe un par contradictorio. El CHECK de BD valida cada columna por separado: un `UPDATE` a mano **sí** puede dejar `Status = cancelled_by_customer` con `CancelledByType = business` (verificado en runtime). Para no repetir los tres literales existe `AppointmentStatuses.Cancellations`.
>
> **Advertencia — penalización económica recortada (RA-869f6ae9h):** ClickUp pedía leer `OrganizationSettings.CancellationHoursThreshold` / `CancellationPenaltyPercentage` y capturar con `IRedsysPaymentService.CaptureAsync`. Ninguna de las dos piezas existe (tabla: **RA-869f2gtyv**; pasarela: **RA-869d7eden**). El bloque pasa de 11 a **12**. La cancelación deja rastro (motivo, fecha, cuenta) y no cobra. Si un proceso automático cancela algún día, habrá que decidir si usa el genérico `cancelled` con tipo nulo.
>
...
>
> **Advertencia — subtarea nueva RA-869f2yh9b (lista de espera).** `WaitingList` se alineó aquí, pero ninguna de las 10 subtareas le daba repositorio, servicio ni endpoints: habría quedado mapeada sin capa de datos, como `ServicePackages` antes de RA-869d7f45n (y el hueco que obligó a crear RA-869f2wtrk). El bloque pasó de 10 a **11**. Más tarde **RA-869f6ae9h** (penalización económica) lo subió a **12**.
>
