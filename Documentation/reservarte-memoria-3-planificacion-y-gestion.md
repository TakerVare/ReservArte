...
> **Módulo Empleados — RA-869d7ed2j (2026-09-13):** **6/10** (el total **no** cambia). **Desglose del numerador (RA-869f18nq5):** shipped = ezrr, ezv0, myx, ezwy, **180e5**, **f043**. **1811u no está hecha** (antes se documentó shipped por error). Pendientes que cuentan: **RA-869d7ezz4** (endpoints; **antes** el catálogo de roles **RA-869f18116** **y** el envelope de challenge/forbidden **RA-869f1anz3**), **RA-869d7f01b** (availability), **RA-869f17y68** (invitación), **RA-869f1811u** (transacción). **RA-869f17y7n** es colateral, fuera del 10.
>
...
>
> **PR #44 (`f37b3a7`) mergeado (2026-09-13).** Caso E2E de **403 sin cuerpo** (la forma real de `[Authorize]`); comentarios de contrato en `client.ts`. Recuento vigente **39/39** (36 + 3, tres navegadores). Unit **100/100**, build 0/0. **No** cierra el hueco del envelope: **RA-869f1anz3 queda abierta**. **Método:** el stub previo del «403 de otro código» llevaba envelope, algo que el 403 de rol **nunca** tendrá; el test pasaba sin cubrir la situación real. Detectado en revisión de documentación, no por la suite.
>
> **Alta en backlog: RA-869f1anz3** — el 401/403 de `[Authorize]` no usa `ApiResponse` (cuerpo vacío). Resolver **antes o junto a** RA-869d7ezz4 y RA-869f18116. Opciones: envolver (`OnChallenge`/`OnForbidden` o `UseStatusCodePages`) o documentar la excepción de forma permanente.
>
...
> **Módulo Empleados (RA-869d7ed2j):** **6/10** (2026-09-13). Numerador (RA-869f18nq5): **RA-869d7ezrr**, **RA-869d7ezv0**, **RA-869f17myx**, **RA-869d7ezwy**, **RA-869f180e5**, **RA-869d7f043**. **RA-869f1811u pendiente** (no shipped). Colateral **RA-869f17y7n** (no cuenta). Pendientes del 10: **RA-869d7ezz4**, **RA-869d7f01b**, **RA-869f17y68**, **RA-869f1811u**. Antes de ezz4: roles **RA-869f18116** y envelope de `[Authorize]` **RA-869f1anz3**. Scripts `data/` vs EF: **RA-869f17mzg**. Query filters del resto: **RA-869f17vet**. `Result<T>` vs `AuthResult<T>`: **RA-869f17y6k**. Detalle: vol. 2 **§9.6**.
...
- [x] Implementar axios client con interceptors (`client.ts`: **401** protegido por status; **403** solo si `error.code` ∈ `SESSION_ENDING_ERROR_CODES`; `endSession()` con `window.location` a propósito — RA-869f18urw / PR #44)
...
- [x] **E2E frontend:** **Playwright** + **`@axe-core/playwright`** en `reservarte-web` (`playwright.config.ts`, tests en `reservarte-web/e2e/`, Chromium / Firefox / WebKit). Scripts `test:e2e`, `test:e2e:ui`, `test:e2e:report`. Humo E2E, **test a11y `LoginPage` (RA-869d7fbpp)**, **retorno OAuth (`e2e/oauth-callback.spec.ts`, RA-869d7f7r1)**, **reset-password (`e2e/reset-password.spec.ts`, RA-869f18rp7)** y **fin de sesión (`e2e/session-ending.spec.ts`, RA-869f18urw + PR #44, incl. 403 sin cuerpo)** verificados (suite **39/39**). Plan previo `tests/ReservArte.E2ETests` **abandonado**. Escenarios de producto E2E **siguen pendientes**. El test a11y **excluye** `color-contrast` (deuda RA-869f0v6vm). El E2E OAuth **no** cubre un IdP real. El flujo forgot→email→reset con backend real: **RA-869f18uta**.
