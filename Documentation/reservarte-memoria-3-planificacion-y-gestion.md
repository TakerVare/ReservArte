...
- ⏳ CRUD de clientes — bloque **RA-869d7ed68: 3/8** (dominio + esquema/repositorio + alta pública; 2026-09-15)
  ...
  - **Alta pública con ficha (RA-869f1xc2n, 2026-09-15)** — **shipped** (PR #59). Registro local: cuenta + ficha `regular` + `data_processing`. Alta social: ficha sin consentimientos granulares. Migración `BackfillCustomerProfiles`. Checkbox `acceptedDataProcessing` en la SPA.
...
- ⏳ Testing unitario de endpoints críticos — **sí** empleados + auth (incl. alta pública, `PublicSignupCustomerTests`); **no** clientes/servicios/citas/pagos. Repositorio de clientes: **sí** (`CustomerRepositoryTests`). Clientes API: **RA-869d7f3q4** (Tests unitarios: crear sin consentimiento debe fallar + bloqueo por no-shows).
...
- [x] **Registro (`RegisterPage`, RA-869d7fbhg)** — **shipped** (2026-08-25); patrón Banner (no `AuthLayout`). Verificado: Zod, consentimiento versionado, login automático. **RA-869f1xc2n (PR #59):** tercer checkbox `acceptedDataProcessing` y ficha `regular` al registrarse. Detalle: vol. 2 **§9.2.3**.
...
- [x] **Backend unitario:** proyecto `tests/ReservArte.UnitTests` con xUnit + Moq + FluentAssertions; suite **246/246** (2026-09-15). Repositorios: SQLite en memoria. `[reservarte-testing-strategy.md](reservarte-testing-strategy.md)` §3.1
