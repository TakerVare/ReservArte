...
> **Contrato de respuestas JSON:** los controladores, `TenantMiddleware`, el rate limiter y los 401/403 de JwtBearer (`OnChallenge` / `OnForbidden`, RA-869f1anz3, 2026-09-14) usan el **envelope** `{ success, data, error, meta }` (vol. 1 **§5.1.1**). Excepciones: webhooks Redsys, health checks, y el hueco 400 `ProblemDetails` de `[ApiController]`. Los ejemplos de este capítulo con `BadRequest(new { success = false, error = "..." })` deben evolucionar al envelope (incl. `meta.requestId`).
...
- **Endpoints (RA-869d7ezz4):** lista `data.items` + `meta.pagination` (`ApiItems<T>`); GET/PUT/DELETE por `{id:int}`; POST 201 + `Location`; **`POST …/{id}/reactivate`**. Mapeo en controlador: `GEN_VALIDATION_FAILED` / `ORG_TENANT_NOT_RESOLVED` → 400; `GEN_FORBIDDEN` → 403; `GEN_NOT_FOUND` → 404; `GEN_CONFLICT` → 409; código sin mapear → **500** (no disfrazarlo de 400). Id no numérico → 404 sin cuerpo (no hay ruta). `ValidateAsync`/`ToCamelCase` duplicados con `AuthController` → unificar en **RA-869f17y6k**. **RA-869d7f01b** (availability) heredará el mismo `[Authorize(Roles = Admin,Manager)]` y las reglas de quién toca a un Admin.
