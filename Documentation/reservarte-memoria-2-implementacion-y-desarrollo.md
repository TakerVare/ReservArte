...
**Aislamiento multi-tenant (RA-869f17vet, PR #54, 2026-09-14; `UserLogin` RA-869f1xc0u, PR #57):** `AppDbContext` recibe `ICurrentOrganizationService`; `CurrentOrganizationId` se lee en los query filters. Constructor de solo opciones: migraciones, seeders y tests (sin tenant, el filtro no restringe). Filtros: disponibilidad, **`Employee`**, **`User`**, **`UserLogin`**, **`RefreshToken`** (`User.OrganizationId`). Claims y tokens de Identity **sin** filtro (justificado). **Cero** `IgnoreQueryFilters()` en producción. Filtro **manual** de `EmployeeRepository` **se mantiene**. Test de metadatos: entidad con `OrganizationId` sin filtro = fallo. Test `AppDbContextTenantResolutionTests`: DI debe elegir el constructor **con** tenant. **Advertencia:** un `Where(OrganizationId == …)` a mano en un repositorio **no** sustituye el filtro global; el global es la protección. El manual en empleados es defensa extra, no la fuente de verdad.
...
**Email** obligatorio en la entidad. Unicidad: vol. 1 **§3.1.3** / **§4.3.1** (por organización, **RA-869f1xc0u** shipped). El índice de `Customers` debe nacer como `(OrganizationId, Email)` en **RA-869d7f32r**.

**Navegaciones** a citas, pagos y lista de espera: no en `Customer`; llegan con esos módulos (criterio `Employee`).

**Nota — `CustomerPaymentMethod.Appointments` / `Payments`:** esas colecciones **no** arrastran `Appointment`/`Payment` al modelo mientras esos tipos sigan en `Ignore`: EF descarta navegaciones hacia tipos ignorados. Evidencia: PR #56 añadió `User.Customer` con `Customer` ignorado y `has-pending-model-changes` no detectó cambios. Se retirarán al mapear **RA-869d7f3fw** solo por coherencia con `Employee` y `Customer`.

**Persistencia:** `Ignore` de `Customer`, `CustomerNote`, `CustomerAllergy`, `CustomerConsent`, `CustomerPaymentMethod`. Verificado sin modelo pendiente. Esquema + filtros: **RA-869d7f32r** (desbloqueada: **RA-869f1xc0u** shipped).

**Archivos `.bak` (no son configuración vigente):** exactamente **tres**, versionados desde el commit `6d2848b` en `ReservArte-Infrastructure/Persistence/Configurations/`: `CustomerConfiguration.cs.bak`, `AppointmentConfiguration.cs.bak`, `EmployeeServiceAssignmentConfiguration.cs.bak`. El de Customer está **obsoleto** (mapea `Rol` e índice único global; el producto vigente es por organización, RA-869f1xc0u). Se eliminan en **RA-869d7f32r**. `EmployeeServiceConfiguration.cs.bak` **no** es un cuarto fichero: es el nombre antiguo del de Assignment; se renombró en `3a3bf2d` (**RA-869f17y7n**).

**Tests:** `CustomerDomainTests` (12). Suite **219/219**.

**Abiertos:** no-shows **RA-869d7f3ka**; categoría `new` + dual ficha **RA-869d7f369**; alta pública **RA-869f1xc2n** (depende de **RA-869d7f32r**); **RA-869d7f3bt** (Endpoints GET/POST/PUT `/api/v1/customers` con búsqueda avanzada y filtros); **RA-869d7f3q4** (Tests unitarios: crear sin consentimiento debe fallar + bloqueo por no-shows). Unicidad **RA-869f1xc0u** (**shipped**, PR #57; independiente, no suma al 8). **Siguiente de esquema: RA-869d7f32r**.
