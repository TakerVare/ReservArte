...
// Alta de usuario con contraseña (AuthService.RegisterAsync, dentro de IUnitOfWork — RA-869f1xc2n)
var result = await _userManager.CreateAsync(user, password);
// CreateAsync hashea con IPasswordHasher<User> y persiste en AspNetUsers.PasswordHash.
// El IdentityResult se comprueba; si falla, la transacción no confirma y no hay ficha Customer.
```
...
### 9.7 Dominio, persistencia y alta pública — módulo de Clientes (RA-869d7f2z5 + RA-869d7f32r + RA-869f1xc2n)
...
**RA-869d7f32r (PR #58, 2026-09-15) — esquema y repositorio.** Migración `20260915112149_AddCustomers` (solo crea tablas; no toca las existentes). `create_ReservArteDB.sql` regenerado en el mismo PR.

**RA-869f1xc2n (PR #59, 2026-09-15) — alta pública con ficha.** `RegisterAsync` y el alta de `ExternalLoginAsync` crean cuenta + ficha `Customer` (mismo Id) en `IUnitOfWork`. Recuento del padre: **3/8**. Cadena: **RA-869f1xc0u** (hecha) → **RA-869d7f32r** (hecha) → **RA-869f1xc2n** (hecha) → **RA-869d7f369**.
...
**Empleada y clienta, misma cuenta (decisión 2026-09-14):** un `User` puede tener **ambas** fichas (`Employee` y `Customer`) con el mismo Id. `User.Rol` es el rol de **personal**. Implementación en **RA-869d7f369**: la ficha Customer no implica `Rol = Customer`; el alta de cliente sobre una cuenta ya existente en el tenant **añade ficha**, no otra cuenta; la baja de ficha de cliente **no** hace lockout (solo la baja de empleado).

**Alta pública con ficha (RA-869f1xc2n, PR #59, 2026-09-15):** `RegisterAsync` y el alta de `ExternalLoginAsync` (`CreateUserAsync`) crean `User` `Customer` **y** fila `Customer` (mismo Id, categoría `regular`) dentro de `IUnitOfWork`. Cada `IdentityResult` se comprueba (`CreateAsync`, y en social también `AddLoginAsync`). `IssueTokensAsync` va **después** del commit. Contrato de registro **intacto** (sin campos nuevos). **Sin** `CustomerConsents.data_processing`: el nivel a no lo implica (vol. 1 §6.1.3). Los caminos OAuth de vínculo existente o email coincidente **no** tocan `Customers`. No hay migración de backfill: `DevSeeder` / `seed_demo` rellenan el demo; cuentas `Customer` antiguas sin ficha no se reparan. Recuento del padre: **3/8**.

**`OrganizationId` es `Guid`** (antes `int` en `Customer`, incompatible con `Organization.Id`). `CustomerNote`, `CustomerAllergy` y `CustomerConsent` llevan `OrganizationId` propio + navegación `Organization` (redundancia deliberada, RA-869f17myx). **`CustomerPaymentMethod` no** tiene `OrganizationId` y **sigue en `Ignore`** — **RA-869d7f3fw** lo añade al mapear. El `organization_id` del sketch `customer_payment_methods` del vol. 1 **§5.2** es el **estado objetivo**; la entidad aún no lo tiene (el sketch no está mal). El test de metadatos lo exigirá al mapear.

**Retirado de `Customer`:** `Rol` (fuente: `User.Rol`) y `MarketingConsent` (fuente: `CustomerConsents`).

**Catálogos** (constantes texto, snake_case minúsculas, como `EmployeeExceptionTypes`; `Roles` es la excepción PascalCase):
- `CustomerCategories`: `regular` (default de entidad), `vip`, `new`. Bloqueo = `IsBlocked` + `BlockedReason` (**nvarchar(500)** en BD; el sketch original del vol. 1 decía `NVARCHAR(MAX)` — alineado en v6).
...
Los CHECK de catálogo se generan desde esas constantes (`CatalogCheck` en Infrastructure); un test fija el SQL resultante. **Sin DEFAULT en BD** (igual que `Employees`): `regular`, `email` e `IsActive = 1` los pone la entidad.
...
**Nota — `CustomerPaymentMethod.Appointments` / `Payments`:** esas colecciones **no** arrastran `Appointment`/`Payment` al modelo mientras esos tipos sigan en `Ignore`: EF descarta navegaciones hacia tipos ignorados. Evidencia histórica: PR #56 añadió `User.Customer` con `Customer` ignorado y `has-pending-model-changes` no detectó cambios. Se retirarán al mapear **RA-869d7f3fw** solo por coherencia con `Employee` y `Customer`.
...
- **Sin `GetHistoryAsync`:** el historial de citas necesita `Appointment`, que no está mapeado. Llega con el módulo de Citas; el endpoint `/history` es de **RA-869d7f3fw**.
...
**Tests:** `CustomerDomainTests` (12) + `CustomerRepositoryTests` + `RegisterCreatesCustomerTests` (SQLite + Identity real: registro con ficha y rollback si `CreateAsync` falla; OAuth alta vs vínculo; sin `data_processing`; email de empleada → `GEN_CONFLICT`; dos orgs el mismo email). Suite **243/243**. E2E **51/51**.
...
**Verificación en runtime (PR #59):** registro 201 + fila `Customers` con el mismo Id, categoría `regular`, sin `CustomerConsents`; segundo registro mismo email 409 y recuentos iguales; login 200. OAuth sin IdP real: cubierto por `RegisterCreatesCustomerTests`.

**Abiertos:** no-shows **RA-869d7f3ka**; categoría `new` + dual ficha **RA-869d7f369** (**siguiente**); **RA-869d7f3bt** (Endpoints GET/POST/PUT `/api/v1/customers` con búsqueda avanzada y filtros); **RA-869d7f3fw** (`OrganizationId` en `CustomerPaymentMethod` y `/history`); **RA-869d7f3q4** (Tests unitarios: crear sin consentimiento debe fallar + bloqueo por no-shows). Unicidad **RA-869f1xc0u** (**shipped**, PR #57; independiente, no suma al 8). Alta pública **RA-869f1xc2n** (**shipped**, PR #59).
