...
- **Alta (usuario → ficha):** `UserManager.CreateAsync` **sin contraseña**, luego `Employee` con `Id = user.Id`. Un usuario huérfano bloquearía el email (índice único global). La justificación del PR #37 para no usar transacción («no tocar el camino de auth») era **floja**: una transacción explícita de EF sobre el mismo `AppDbContext` no exige cambiar `AuthService`. Corrección: **RA-869f1811u**. En este árbol el alta aún muestra reversión manual (`DeleteAsync` en el `catch`) si 1811u no ha aterrizado.
- **Edición:** propaga nombre, email (`SetEmailAsync` + `SetUserNameAsync`), teléfono, rol e imagen a Identity. Sin transacción compartida, ficha y cuenta pueden desfasarse; entra en el mismo hueco que cubre **RA-869f1811u**.
