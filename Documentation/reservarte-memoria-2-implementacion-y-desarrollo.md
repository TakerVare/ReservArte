...
**Abiertos del bloque (1):** **RA-869d7f3ka** (`NoShowCount`, política, `IncrementNoShowAsync` y test de bloqueo al umbral). Decisiones abiertas: de dónde sale el umbral (no existe `OrganizationSettings`; `CancellationPolicy` sigue en `Ignore` con `OrganizationId` int), si se crea `AuditLog` (no existe) y qué dispara el no-show (llega con Citas). Historial **RA-869f2gn91** (Citas, no cuenta en el 7). Tarjetas **RA-869f2gnbm** (Redsys, no cuenta en el 7). Promoción de categoría **RA-869f2g02q** (Citas). Notas **RA-869d7f3fw** (**shipped**, PR #62). Endpoints **RA-869d7f3bt** (**shipped**, PR #61). Servicio **RA-869d7f369** (**shipped**, PR #60). **RA-869d7f3q4 cancelada**. Unicidad **RA-869f1xc0u** (**shipped**, PR #57; independiente, no suma al 7).
