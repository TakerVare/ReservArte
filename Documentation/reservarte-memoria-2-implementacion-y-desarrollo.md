...
> **Contrato de respuestas JSON:** los controladores usan el **envelope** `{ success, data, error, meta }` y `error.code` del volumen 1 (**§5.1.1** y **§5.1.2**). **Excepción actual:** el 401/403 de `[Authorize]` sale del middleware JWT **sin cuerpo** (**RA-869f1anz3**). Los ejemplos de este capítulo que usan `BadRequest(new { success = false, error = "..." })` deben evolucionar al envelope (incl. `meta.requestId`).
...
- **Cliente HTTP (`src/lib/api/client.ts`, RA-869f18urw + PR #44):** el interceptor **combina dos criterios**. (1) **401** en endpoint protegido → fin de sesión **por status**; excepciones `AUTH_ENDPOINTS_WITHOUT_SESSION` (`login`, `mfa/verify`, `refresh-token`), cuyo 401 es resultado de negocio **con** envelope. (2) **403** → fin de sesión **solo si** `error.code` ∈ `SESSION_ENDING_ERROR_CODES` (hoy `ORG_TENANT_MISMATCH`). **Punto de extensión y regla para RA-869d7ezz4:** los códigos de **permiso no entran nunca** en esa lista («no tienes rol» deniega la operación, no invalida la sesión). Hoy el 403 de `[Authorize(Roles=…)]` llega **sin cuerpo** (middleware ASP.NET Core, RA-869f1anz3), así que ni hay `error.code`: no cerrar sesión es correcto por partida doble.
...
- **Éxito:** `authStore.login({ accessToken, refreshToken, mfaRequired: false })`; como el fragmento no trae el usuario, se completa con `GET /api/v1/account/me`. Si `/me` falla **sin** código de fin de sesión (incluido 403 **sin cuerpo**, forma real de `[Authorize]`), la sesión **no** se aborta y se redirige a `/`. Si el código **sí** está en la lista (`ORG_TENANT_MISMATCH`), el interceptor **sí** cierra la sesión. Spec `e2e/session-ending.spec.ts` (RA-869f18urw; caso sin cuerpo: PR #44).
...
- **Alta (usuario → ficha):** `UserManager.CreateAsync` **sin contraseña**, luego `Employee` con `Id = user.Id`. Un usuario huérfano bloquearía el email (índice único global). La justificación del PR #37 para no usar transacción («no tocar el camino de auth») era **floja**. **RA-869f1811u** (pendiente, cuenta en el 10 del bloque): transacción explícita de EF; **no está hecha**. El alta sigue con reversión manual (`DeleteAsync` en el `catch`).
- **Edición:** propaga nombre, email (`SetEmailAsync` + `SetUserNameAsync`), teléfono, rol e imagen a Identity. Sin transacción compartida, ficha y cuenta pueden desfasarse; mismo hueco **RA-869f1811u** (pendiente).
