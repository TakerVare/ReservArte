...
- **Cliente HTTP (`src/lib/api/client.ts`, RA-869f18urw; comentarios PR #44–#45):** enumeración, no una sola regla.
  1. **401** en endpoint protegido → fin de sesión **por status**. Exceptuados `AUTH_ENDPOINTS_WITHOUT_SESSION` (`login`, `mfa/verify`, `refresh-token`, **`set-password`**): 401 de negocio **con** envelope. **Hueco (sin ID):** `reset-password` **no** está en la lista; su 401 de enlace caducado cerraría una sesión abierta.
...
- **Alta (usuario → ficha):** `UserManager.CreateAsync` **sin contraseña**, luego `Employee` con `Id = user.Id`. Un usuario huérfano bloquearía el email (índice único global). La justificación del PR #37 para no usar transacción («no tocar el camino de auth») era **floja**. **RA-869f1811u** (pendiente, **único** ítem que queda del 10): transacción explícita de EF; **no está hecha**. El alta sigue con reversión manual (`DeleteAsync` en el `catch`).
- **Invitación (RA-869f17y68, 2026-09-14):** `CreateAsync` envía el correo **después** de crear la ficha. Fallo de `IEmailService` → se registra y **no** revierte el alta. Reenvío: `ResendInvitationAsync` / `POST …/invitation`. **Advertencia para RA-869f1811u:** el envío **no** debe ir dentro de la transacción ni condicionar el commit; si el correo sale antes de un rollback, el empleado recibiría un enlace a una cuenta que ya no existe. Tras commit: enviar (o fallar y dejar reenvío).
- **Advertencia — `EmailConfirmed`:** ni el alta ni el canje `set-password` ponen `EmailConfirmed = true` (el alta social sí). Tarea **RA-869f1812p**; no se adelantó. `ForgotPasswordAsync` no lo exige hoy; si más adelante se activa `RequireConfirmedEmail`, el flujo de invitación se rompe.
