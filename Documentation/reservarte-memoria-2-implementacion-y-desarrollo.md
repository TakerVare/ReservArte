...
- **Unicidad del email: global, no por organización.** `EmailExistsAsync` consulta **sin** filtrar por tenant a propósito: el índice único de `Employees.Email` es global. **Límite:** no mira `AspNetUsers` (cuentas sin ficha); Identity sí, y ahora se respeta (`GEN_CONFLICT`). Unificar el pre-chequeo es opcional (**sin tarea**). Permitir el mismo email en organizaciones distintas exigiría un índice compuesto `(OrganizationId, Email)`: decisión de producto **pendiente**, no tomada.
...
- **Límites (sin tarea):** (1) no hay token de concurrencia en `Employee` — dos ediciones simultáneas: gana el último que escribe; edición/baja cargan la ficha **antes** de la transacción y la re-adjuntan con `Update` en cada reintento. (2) `EmailExistsAsync` **sigue** mirando solo `Employees`; ya no causa daño porque se respeta Identity. (3) el reintento transitorio no se ejercita en tests.
- **Patrón para próximos módulos:** escrituras que abarquen varias tablas, y en especial Identity + tablas propias, por `IUnitOfWork` y **comprobar cada `IdentityResult`**. No ignorar el resultado y luego `SaveChanges` sobre el mismo contexto.
