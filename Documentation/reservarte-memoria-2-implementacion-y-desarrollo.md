...
> **Contrato de respuestas JSON:** los controladores, `TenantMiddleware` y el rate limiter usan el **envelope** `{ success, data, error, meta }` (vol. 1 **§5.1.1**). **Sin envelope hoy (RA-869f1anz3):** (a) 401 challenge JwtBearer de `[Authorize]`; (b) 403 del middleware de autorización `[Authorize(Roles)]`. El 403 `ORG_TENANT_MISMATCH` y el 401 `AUTH_*` de controladores **sí** llevan envelope. Los ejemplos de este capítulo con `BadRequest(new { success = false, error = "..." })` deben evolucionar al envelope (incl. `meta.requestId`).
...
- **Cliente HTTP (`src/lib/api/client.ts`, RA-869f18urw; comentarios PR #44–#45):** enumeración, no una sola regla.
  1. **401** en endpoint protegido → fin de sesión **por status**. Exceptuados `AUTH_ENDPOINTS_WITHOUT_SESSION` (`login`, `mfa/verify`, `refresh-token`): 401 de negocio **con** envelope.
  2. **403** → fin de sesión **solo si** `error.code` ∈ `SESSION_ENDING_ERROR_CODES` (hoy `ORG_TENANT_MISMATCH`, envelope de `TenantMiddleware`).
  3. Códigos de **permiso no entran** en esa lista (condiciona RA-869d7ezz4). El 403 de `[Authorize(Roles)]` **hoy va sin cuerpo**; el spec cubre también un 403 **con** envelope de otro código como escenario de **RA-869f1anz3**, no como el 403 de rol actual.
...
- **Éxito:** `authStore.login({ accessToken, refreshToken, mfaRequired: false })`; como el fragmento no trae el usuario, se completa con `GET /api/v1/account/me`. Si `/me` falla **sin** código de fin de sesión —403 sin cuerpo de `[Authorize]`, u otro código con envelope— la sesión **no** se aborta y se redirige a `/`. Si el código es `ORG_TENANT_MISMATCH`, el interceptor **sí** cierra la sesión. Spec `e2e/session-ending.spec.ts` (cabecera y nombres de caso alineados en **PR #45**).
