...
- **Unicidad del email (código vigente vs producto):** el código **sigue imponiendo unicidad global**: índices `Employees.Email`, `EmailIndex` / `UserNameIndex` (`UserName` = email); **`EmailExistsAsync` + `IgnoreQueryFilters()`**; **`GlobalUniqueUserValidator`**; PK de `AspNetUserLogins` `(LoginProvider, ProviderKey)` (impediría el mismo login social en dos centros). **Decisión de producto (2026-09-14):** único **por organización**. **No implementada.** **Siguiente implementación: RA-869f1xc0u** (independiente, backend, alta), prerrequisito de **RA-869d7f32r**. **Alcance explícito:** índices `(OrganizationId, NormalizedEmail)`, `(OrganizationId, NormalizedUserName)` y `(OrganizationId, Email)` de `Employees`; sustituir la PK global de `AspNetUserLogins`; retirar `GlobalUniqueUserValidator` y el `IgnoreQueryFilters` de `EmailExistsAsync`; auditar caminos **sin tenant** (seeders, jobs) donde `FindByEmailAsync` podría encontrar dos cuentas. **Al implementar:** migración + regenerar `data/schema/create_ReservArteDB.sql` en el mismo PR (RA-869f17mzg); no hay CI que falle si se olvida. La plantilla de PR se actualizará **en ese mismo PR** (mitigación prevista; **hoy no** incluye esa casilla). No presentar lo global como el modelo resuelto.
...
- **Límites (sin tarea, salvo unicidad):** (1) no hay token de concurrencia en `Employee`. (2) unicidad de email **aún global** (ver viñeta de unicidad y **RA-869f1xc0u**). (3) el reintento transitorio no se ejercita en tests.
...
**Email** obligatorio en la entidad. Unicidad: vol. 1 **§3.1.3** / **§4.3.1** (decisión por org, código aún global; **RA-869f1xc0u**).
...
**Persistencia:** `Ignore` de `Customer`, `CustomerNote`, `CustomerAllergy`, `CustomerConsent`, `CustomerPaymentMethod`. Verificado sin modelo pendiente. Esquema + filtros: **RA-869d7f32r** (tras **RA-869f1xc0u**).
