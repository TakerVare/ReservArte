...
- **Cliente HTTP (`src/lib/api/client.ts`, RA-869f18urw):** dos disparadores de fin de sesión. (1) **401** en endpoint protegido (sesión inválida o expirada); se excluyen login, `mfa/verify` y `refresh-token` (`AUTH_ENDPOINTS_WITHOUT_SESSION`). (2) **403 cuyo `error.code` está en `SESSION_ENDING_ERROR_CODES`** (hoy solo `ORG_TENANT_MISMATCH`). La lista es el **punto de extensión**: un 403 nuevo solo cierra sesión si se añade el código a propósito. Limpia `localStorage['authToken']` y redirige a `/login` con `window.location` (acceso directo a `localStorage`, no `authStore`, para no crear dependencia circular store↔client; coherente con el 401 ya existente).
...
- **Éxito:** `authStore.login({ accessToken, refreshToken, mfaRequired: false })`; como el fragmento no trae el usuario, se completa con `GET /api/v1/account/me`. Si `/me` falla con un código que **no** está en `SESSION_ENDING_ERROR_CODES`, la sesión **no** se aborta (los datos de perfil se recargan más tarde) y se redirige a `/`. Si el código **sí** está en la lista (`ORG_TENANT_MISMATCH`), el interceptor **sí** cierra la sesión. Spec `e2e/session-ending.spec.ts` (RA-869f18urw).
