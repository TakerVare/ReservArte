...
- **Unicidad del email (código vigente vs producto):** el código **sigue imponiendo unicidad global**: índices `Employees.Email`, `EmailIndex` / `UserNameIndex` (`UserName` = email); **`EmailExistsAsync` + `IgnoreQueryFilters()`**; **`GlobalUniqueUserValidator`**; PK de `AspNetUserLogins` `(LoginProvider, ProviderKey)` (impediría el mismo login social en dos centros). **Decisión de producto (2026-09-14):** único **por organización**. **No implementada.** Tarea **RA-<pendiente-email>** (backend, alta), prerrequisito de **RA-869d7f32r**. **Alcance explícito:** índices `(OrganizationId, NormalizedEmail)`, `(OrganizationId, NormalizedUserName)` y `(OrganizationId, Email)` de `Employees`; sustituir la PK global de `AspNetUserLogins`; retirar `GlobalUniqueUserValidator` y el `IgnoreQueryFilters` de `EmailExistsAsync`; auditar caminos **sin tenant** (seeders, jobs) donde `FindByEmailAsync` podría encontrar dos cuentas. No presentar lo global como el modelo resuelto.
...
- **Límites (sin tarea, salvo unicidad):** (1) no hay token de concurrencia en `Employee`. (2) unicidad de email **aún global** (ver viñeta de unicidad y **RA-<pendiente-email>**). (3) el reintento transitorio no se ejercita en tests.
...
**Alta pública sin ficha (hueco, RA-<pendiente-alta>):** `RegisterAsync` y el alta de `ExternalLoginAsync` crean `User` con `Rol = Customer` **sin** fila `Customer` y **sin** transacción. Subtarea del bloque **RA-869d7ed68**, prioridad alta; **depende de RA-869d7f32r y de RA-<pendiente-email>**. Alcance: cuenta + ficha (mismo Id) en `IUnitOfWork`, cada `IdentityResult`; backfill en migración; decisión de consentimiento `data_processing` vs registro nivel a (vol. 1 §6.1.3). ClickUp: el recuento del padre sigue **1/7** hasta que exista ID real; al crear la subtarea el denominador puede pasar a **1/8**.
...
**Email** obligatorio en la entidad. Unicidad: vol. 1 **§3.1.3** / **§4.3.1** (decisión por org, código aún global; **RA-<pendiente-email>**).
...
**Abiertos:** no-shows **RA-869d7f3ka**; categoría `new` + dual ficha **RA-869d7f369**; alta pública **RA-<pendiente-alta>**; unicidad **RA-<pendiente-email>**.
