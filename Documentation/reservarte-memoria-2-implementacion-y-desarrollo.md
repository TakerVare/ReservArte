...
### 9.7 Dominio y persistencia — módulo de Clientes (RA-869d7f2z5 + RA-869d7f32r)
...
**RA-869d7f32r (PR #58, 2026-09-15) — esquema y repositorio.** Migración `20260915112149_AddCustomers` (solo crea tablas; no toca las existentes). `create_ReservArteDB.sql` regenerado en el mismo PR. Recuento del padre: **2/8**. Cadena: **RA-869f1xc0u** (hecha) → **RA-869d7f32r** (hecha) → **RA-869f1xc2n** (siguiente).
...
**Alta pública sin ficha (hueco, RA-869f1xc2n):** `RegisterAsync` y el alta de `ExternalLoginAsync` crean `User` con `Rol = Customer` **sin** fila `Customer` y **sin** transacción. Subtarea del bloque **RA-869d7ed68**, prioridad alta; **siguiente** de la cadena (**RA-869f1xc0u** hecha → **RA-869d7f32r** hecha → **RA-869f1xc2n**). Alcance: cuenta + ficha (mismo Id) en `IUnitOfWork`, cada `IdentityResult`; backfill agrupando por `(OrganizationId, email)` (no por email global); decisión de consentimiento `data_processing` vs registro nivel a (vol. 1 §6.1.3). Recuento del padre: **2/8**.
...
**Abiertos:** no-shows **RA-869d7f3ka**; categoría `new` + dual ficha **RA-869d7f369**; alta pública **RA-869f1xc2n** (**siguiente**); **RA-869d7f3bt** (Endpoints GET/POST/PUT `/api/v1/customers` con búsqueda avanzada y filtros); **RA-869d7f3fw** (`OrganizationId` en `CustomerPaymentMethod` y `/history`); **RA-869d7f3q4** (Tests unitarios: crear sin consentimiento debe fallar + bloqueo por no-shows). Unicidad **RA-869f1xc0u** (**shipped**, PR #57; independiente, no suma al 8).
